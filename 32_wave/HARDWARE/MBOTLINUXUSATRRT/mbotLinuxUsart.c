...
int usartReceiveOneData(int *p_leftSpeedSet,int *p_rightSpeedSet, unsigned char *p_crtlFlag)
{
	...
	static short dataLength                   = 0;					//静态局部变量是在编译时赋初值的，且只赋初值一次，
	//在程序运行时它已有初值。以后在每次调用函数时就不再重新赋初值，而是保留上次函数调用结束时的值。
	...
	if(Start_Flag == START)
	{
		...
		{  
			if(USARTReceiverFront == 0x55)        //数据头两位 //buf[0]
			{
				...
				receiveBuff[1]=header[1];         //buf[1]		保证缓存区和实际数据结构一致
...
返回  值：无
**************************************************************************/
void usartSendData(short leftVel, short rightVel,float distance, short angle,unsigned char ctrlFlag)
{
	// 协议数据缓存数组
	unsigned char buf[13+4] = {0};
	int i, length = 0;
	...
	for(i = 0; i < 2; i++)
	{
		...
		buf[i + 9] = distanceNow.data[i];        // buf[9] buf[10]
		buf[i + 11] = distanceNow.data[i+2];        // buf[11] buf[12]
	}
	// 预留控制指令
	buf[3 + length - 1] = ctrlFlag;              // buf[9+4]
	
	// 设置校验值、消息尾

	
	buf[3 + length] = getCrc8(buf, 3 + length);  // buf[10+4]
	buf[3 + length + 1] = ender[0];              // buf[11+4]
	buf[3 + length + 2] = ender[1];              // buf[12+4]
