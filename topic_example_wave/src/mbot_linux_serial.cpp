...
出口参数：
********************************************************/
void writeSpeed(double Left_v, double Right_v,double testDistance,unsigned char ctrlFlag)
{
    ...
    // 设置校验值、消息尾
    buf[3 + length] = getCrc8(buf, 3 + length);//buf[8+4]
    buf[3 + length + 1] = ender[0];     //buf[9+4]
    buf[3 + length + 2] = ender[1];     //buf[10+4]
...
出口参数：bool
********************************************************/
bool readSpeed(double &Left_v,double &Right_v,double &distance,double &Angle,unsigned char &ctrlFlag)
{
    char i, length = 0;
    ...
    for(i = 0; i < 2; i++)
    {
        ...
        distanceNow.data[i] = buf[i + 9];
        distanceNow.data[i+2]=buf[i + 11];
